...
Если кратко, то собирать нужно на x86-компьютере с Linux'ом.
Если вы хотите собрать на другой ОС, нужно изменить Makefile чтобы скомпилировать `sh` и `vi` используя кросс-компилятор, который генерирует ELF-бинарники -- то есть кросс-компилятор с target'ом x86 Linux. Также, нужен очень старый gcc с версией около 3.3.5.
